...
st.markdown("""
Enter a symbolic expression like `1*(2+3+...+10)` or `x_1*(x_2+x_3+x_4)` to generate a star graph.
""")
...
if st.button("Draw Graph"):
    ...
    if not edges:
        st.error("Invalid expression format. Please use formats like 1*(2+3+4), x_1*(2+3+...+10), or A*(B+C+D).")
    else:
        G = nx.Graph()
